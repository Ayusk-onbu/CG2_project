...
void Teapot::Initialize(ModelObject& model, CameraBase& camera) {
	OBJ::Initialize(model, camera);
	worldTransform_.set_.Translation({ 5.0f,1.0f,0.0f });
	worldTransform_.set_.Rotation({ 0.0f,Deg2Rad(0),0.0f });
}
void Teapot::Update() {
...
void Teapot::Draw(TheOrderCommand& command, PSO& pso, DirectionLight& light, Texture& tex) {
