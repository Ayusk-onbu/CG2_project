...
void ObjectData::MakeObjectData(Fngine* fngine, const std::vector<VertexData>& vertices, const std::vector<uint32_t>& indices) {
	auto d3d12 = fngine->GetD3D12System();
	//   ========
	// 【 Vertex 】
	//   ========
	...
	//   ========
	if (indices.size() > 0) {
		indexResource_ = CreateBufferResource(d3d12.GetDevice().Get(), sizeof(uint32_t) * indices.size());
