...
void HairClosestHitShader(inout RayPayload payload, in HairAttribute attribute)
{
    ...
    // ------------------------------------------------------------
    float32_t roughness = 0.35f; // 髪の粗さ（β）
    float32_t shift = 0.1f; // キューティクルの傾き（α）
    ...
    float32_t specR_intensity = 0.6f; // Rスペキュラ全体を60%の強度に抑える
    float32_t3 specR = float32_t3(1.0f, 1.0f, 1.0f) * M_R * fresnel * specR_intensity;
