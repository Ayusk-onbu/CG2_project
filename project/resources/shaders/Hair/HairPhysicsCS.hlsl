...
void ApplyConstraint(uint parentIdx, uint childIdx)
{
    ...
    float targetLength = g_GuideBuffer[parentIdx].nextToLength; // 本来の長さ
...
{   
    ...
    if (info.vertexCount < 2)return; // 頂点が2個未満ならセグメントを作れないので終了
    ...
    //   ========================================
        ...
        // 風の計算（時間と座標でサイン波を作ってなびかせる）：なにか計算がおかしく、動く方向の座標が反対になる
    ...
    {
       // 順方向パス (根元 -> 毛先)
        // 💡固定値ではなく、このガイド固有の vertexCount を使う！
        ...
        // 逆方向パス (毛先 -> 根元)
        // 💡ここも全体の固定値ではなく、vertexCount を基準に毛先から逆算する！
