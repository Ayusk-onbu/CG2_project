...
void ModelObject::Draw(TheOrderCommand& command, PSO& pso, DirectionLight& light, Texture& tex, uint32_t num) {
	command.GetList().GetList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	object_.DrawBase(command, pso, light, tex);
	command.GetList().GetList()->DrawInstanced(UINT(modelData_.vertices.size()), num, 0, 0);
}
