...
void MultiMaterial::Initialize(ModelObject& model, CameraBase& camera) {
	OBJ::Initialize(model, camera);
	worldTransform_.set_.Translation({ 5.0f,5.0f,0.0f });
...
void MultiMaterial::Draw(TheOrderCommand& command, PSO& pso, DirectionLight& light, Texture& tex) {
