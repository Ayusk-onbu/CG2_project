...
void Player::Initialize(ModelObject* model, Vector3& pos) {
	model_ = model;
...
#pragma region ⑧行列計算
	//// 行列更新
	//SetWorldTransform(transform_);
#pragma endregion
	////////////////////////////////////////////////////

}

void Player::Draw(Camera& camera, TheOrderCommand& command, PSO& pso, DirectionLight& light, Texture& tex) {
	//ゲームの処理
...
void Player::Draw(DebugCamera& camera, TheOrderCommand& command, PSO& pso, DirectionLight& light, Texture& tex) {
	//ゲームの処理
	Matrix4x4 worldMatrix = Matrix4x4::Make::Affine(transform_.scale, transform_.rotate, transform_.translate);
	Matrix4x4 uvTransformMatrix = Matrix4x4::Make::Scale(uvTransform_.scale);
	uvTransformMatrix = Matrix4x4::Multiply(uvTransformMatrix, Matrix4x4::Make::RotateZ(uvTransform_.rotate.z));
...
void Player::OnCollision(const Enemy* enemy) {
	(void)enemy;
	velocity_ = velocity_ +Vector3(0.0f, 0.1f, 0.0f);
	isDeath_ = true;
}
